...
function getUbuntuVm(){
    if ! virsh list | grep "$VM_NAME"; then
        ...
        chmod +x ./kvm-on-machine/kvm-install.sh
        sudo ./kvm-on-machine/kvm-install.sh --ubuntu --full

        rm -rf kvm-on-machine
    else
...
function diskConfigure() {
    if ! sudo vgs | grep vg0 &>/dev/null; then
...
function fixClassicAptList() {
    if [[ "$1" == "--undo" ]]; then
        ...
        [[ "$APT_UPDATED" != "1" ]] && sudo apt update && APT_UPDATED=1
        sudo mv /etc/apt/sources.list.d/ubuntu.sources.disabled \
...
function checkXen () {
    if ! dpkg -s "xen-tools" &>/dev/null; then
        ...
        [[ "$APT_UPDATED" != "1" ]] && sudo apt update &>/dev/null && APT_UPDATED=1
...
function addLocaltime() {
    ...
    if ! cat /etc/xen/$hostname.pv | grep "localtime" &>/dev/null; then
        echo "localtime = 1" | sudo tee -a /etc/xen/$hostname.pv >/dev/null
    else
